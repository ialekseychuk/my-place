...
export function Widget({ businessId, locationId: propLocationId, businessName = 'Мой Салон', businessAddress = 'ул. Примерная, д.1' }: WidgetProps) {
  ...
  const [staffTimeSlots, setStaffTimeSlots] = useState<Record<string, string[]>>({})
  ...
  // Update masters
  useEffect(() => {
    if (!selectedLocationId || !allStaffs.length || !staffServiceAssignments.length) {
      setStaffs(allStaffs)
      return
    }
  ...
  // Update services
  useEffect(() => {
    if (!selectedLocationId || !allServices.length || !staffServiceAssignments.length) {
      ...
      try {
        ...
        const schedules = await Promise.all(
          targetStaffIds.map(async staffId => {
            ...
            const existingBookings = await widgetService.getStaffBookings(businessId, staffId, selectedDate)
            const serviceObj = selectedService ? allServices.find(s => s.id === selectedService) : undefined
            const allTimes = generateAllTimesFromSchedule(schedule, serviceObj?.duration_min, existingBookings, selectedLocation.timezone)
            return { staffId, available: allTimes.available, booked: allTimes.booked }
          })
        )
        ...
        schedules.forEach(({ staffId, available, booked }) => {
          timeSlotsMap[staffId] = available
          bookedTimeSlotsMap[staffId] = booked
          allAvailableTimes.push(...available)
        ...
        setBookedTimeSlots(bookedTimeSlotsMap)
      } catch (error) {
          ...
          for (let time = new Date(start); time.getTime() + slotDuration <= end.getTime(); time.setMinutes(time.getMinutes() + 30)) {
            ...
            if (existingBookings && existingBookings.length > 0) {
              ...
              for (const booking of existingBookings) {
                ...
                const bookingStart = new Date(startYear, startMonth - 1, startDay, startHour, startMinute, startSecond, 0)
                
                const [endDatePart, endTimePart] = bookingEndStr.split('T')
  ...
  const proceedToConfirmation = () => {
    if (selectedService && selectedStaff && selectedDate && selectedTime && isValidSelection()) {
